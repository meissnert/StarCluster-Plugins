...
class FakeModInstaller(ClusterSetup):
	def run(self, nodes, master, user, user_shell, volumes):
		for node in nodes:
			...
			node.ssh.execute('mkdir -p /usr/local/Modules/applications/python')
			...
			node.ssh.execute('mkdir -p /usr/local/Modules/applications/mutect')
			...
			log.info("Creating MuTect 1.1.7 module files on %s" % (node.alias))
			node.ssh.execute('mkdir -p /usr/local/Modules/applications/mutect')
			node.ssh.execute('echo "#%Module" >> /usr/local/Modules/applications/mutect/1.1.7')
			node.ssh.execute('echo "set root /opt/software/mutect/1.1.7" >> /usr/local/Modules/applications/mutect/1.1.7')
			node.ssh.execute('echo -e "prepend-path\tPATH\t\$root" >> /usr/local/Modules/applications/mutect/1.1.7')
			node.ssh.execute('mkdir -p /opt/software/mutect/1.1.7')