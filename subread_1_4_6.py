...
class SubreadInstaller(ClusterSetup):
	def run(self, nodes, master, user, user_shell, volumes):
		for node in nodes:
			...
			node.ssh.execute('wget -c -P /opt/software/subread/1.4.6 http://sourceforge.net/projects/subread/files/subread-1.4.6/subread-1.4.6-source.tar.gz')
			node.ssh.execute('cd /opt/software/subread/1.4.6/ && tar xvzf subread-1.4.6-source.tar.gz')
